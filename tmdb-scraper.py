...
async def scrape_all_ids():
    ...
    ps3_physical = [f"B{rights}{region}{rtype}" for rights, region, rtype in product("CL", "ACEHJKU", "BCDMSX") if not (rtype == "M" and region != "J")]
    ps3_mrtc = [f"MRTC"]
    ...
    async with aiohttp.ClientSession() as session:
        for prefixes, path, ext in [
            (psx_ps2_physical + ps3_physical + ps3_digital + ps3_mrtc, "tmdb", "xml"), # PSP and VITA may use this one too?
